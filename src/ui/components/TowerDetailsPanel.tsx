...
export function TowerDetailsPanel({
  ...
  const nextStats: Partial<TowerProfile> | null = nextUpgrade
    ? {
        ...
        fireRate: Math.max(
          0.1,
          towerProfile.fireRate / nextUpgrade.statMultiplier
        ),
        projectileSpeed: Math.round(
          towerProfile.projectileSpeed * nextUpgrade.statMultiplier
        ),
      }
    : null;
      ...
      <div className="tower-details-content">
        ...
        <div className="tower-details-stats">
          ...
          <div className="stat-item">
            ...
            <span className="stat-value">
              {towerProfile.fireRate.toFixed(2)}s
            </span>
          </div>
        </div>
        ...
        <div className="tower-details-dps">
          ...
          <span className="dps-value">
            {(towerProfile.damage / towerProfile.fireRate).toFixed(1)}
          </span>
          <span className="dps-efficiency">
            ($
            {(
              (towerProfile.damage /
                towerProfile.fireRate /
                towerProfile.cost) *
              100
            ).toFixed(1)}{' '}
            DPS/$){' '}
        ...
        <div className="tower-details-meta">
          ...
          {towerProfile.splashRadius ? (
            <span className="pill">Splash</span>
          ) : null}
          ...
          {nextStats ? (
            <div className="upgrade-body">
              <div className="upgrade-cost">
                Cost: ${upgradeInfo.upgradeCost}
              </div>
              <div className="upgrade-grid">
                <div className="upgrade-stat">
                  <span>Range</span>
                  <strong>
                    {towerProfile.range} → {nextStats.range}
                  </strong>
                </div>
                <div className="upgrade-stat">
                  <span>Damage</span>
                  <strong>
                    {towerProfile.damage} → {nextStats.damage}
                  </strong>
                </div>
                <div className="upgrade-stat">
                  <span>Fire Rate</span>
                ...
                <div className="upgrade-stat">
                  ...
                  <strong>
                    {towerProfile.projectileSpeed} → {nextStats.projectileSpeed}
                  </strong>
                </div>
              </div>
            ...
            <p className="maxed">
              Max level reached on build menu. Use in-game upgrades per tower.
            </p>
          )}
        </div>
      </div>
