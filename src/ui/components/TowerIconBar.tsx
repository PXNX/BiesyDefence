...
export function TowerIconBar({
  ...
  const handleKeyNavigation = (event: React.KeyboardEvent) => {
    ...
    if (event.key === 'ArrowRight' || event.key === 'ArrowLeft') {
      ...
      const nextIndex = (currentIndex + direction + TOWER_ENTRIES.length) % TOWER_ENTRIES.length
  ...
  const wrapperClassName = [
    'tower-icon-bar',
    orientation === 'vertical' ? 'tower-icon-bar-vertical' : 'tower-icon-bar-horizontal',
    className,
  ]
  ...
  return (
    <div className={wrapperClassName.filter(Boolean).join(' ')}>
      ...
      {/* Feedback Messages */}
      {feedback && (
        <div className={`tower-feedback ${feedback.includes('Insufficient') ? 'error' : 'success'}`} role="alert">
          {feedback}
        </div>
      )}
    </div>
