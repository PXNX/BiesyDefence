...
export function UpgradePanel({
  ...
  const levelCost =
    level === 1 ? baseCosts.level2 : level === 2 ? baseCosts.level3 : null;
  const perks = plan?.perks ?? [];

  const branchFilter = branch
    ? (p: any) => p.branch === branch
    : (p: any) => true;
  const branchLocked = (perkBranch: BranchId) =>
    branch && branch !== perkBranch;
      ...
      <section className="upgrade-panel__core">
        <div className="core-row">
          <div className="core-label">
            <img
              src="/ui/icons/perks/ui_upgrade_arrow.png"
              alt=""
              aria-hidden="true"
            />
            ...
            return (
              ...
              >
                <div className="perk-head">
                  ...
                  <span className={`branch-chip branch-${perk.branch}`}>
                    Branch {perk.branch}
                  </span>
                </div>
                ...
                <div className="perk-meta">
                  ...
                  <span className="perk-status">
                    {lockedBranch ? 'Locked' : owned ? 'Owned' : ''}
                  </span>
                </div>
              </button>
            );
