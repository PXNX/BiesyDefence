...
export function TowerRadialMenu({
  hoverTower,
  money,
  onUpgradeLevel,
  onBuyPerk,
}: TowerRadialMenuProps) {
  ...
  const branchSelected = hoverTower.upgradeState?.branch as
    | BranchId
    | undefined;
      ...
      [
        {
          ...
          title: maxedLevel
            ? 'Max Level erreicht'
            : `Upgrade auf L${level + 1} für ${formatCost(hoverTower.nextCost)}`,
        ...
        },
        {
          ...
          cost: perkA?.cost,
          onClick: () =>
            perkA && !lockedA && onBuyPerk(hoverTower.id, perkA.id),
        ...
        },
        {
          ...
          cost: perkB?.cost,
          onClick: () =>
            perkB && !lockedB && onBuyPerk(hoverTower.id, perkB.id),
    ...
    <div className="tower-radial" style={centerStyle}>
      <div className="radial-center">
        ...
        <div className="radial-stats">
          <img
            src="/ui/icons/perks/critical_hit_icon.png"
            alt="crit"
            title="Crit/Weakpoint"
          />
          <img
            src="/ui/icons/perks/dot_damage_icon.png"
            alt="dot"
            title="DoT/Status"
          />
          <img
            src="/ui/icons/perks/range_upgrade_icon.png"
            alt="range"
            title="Range/Control"
          />
        </div>
      ...
      {actions.map((item, idx) => {
        ...
        const isReady =
          !item.disabled && item.cost != null && item.cost <= money;
        const isBranchLocked =
          item.className.includes('locked') &&
          item.className.includes('branch');
        ...
        return (
          ...
          >
            <span className="node-chip">
              {item.key === 'core' ? 'C' : item.key === 'branch-a' ? 'A' : 'B'}
            </span>
