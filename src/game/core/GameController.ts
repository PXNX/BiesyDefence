...
import { createTowerUpgradeSystem, type TowerUpgradeSummary } from '@/game/systems/TowerUpgradeSystem'
...
export class GameController {
    ...
    private update(deltaSeconds: number): void {
        ...
        this.systemManager.updateSystems(this.state, deltaSeconds, this.modifierManager, modifierEvents, {
            ...
            onEnemyDeath: (enemyId, killerId) => this.handleEnemyDeath(enemyId, killerId),
        })
    ...
    sellTower(towerId: string): PlacementResult {
        ...
        const refund = Math.floor(profile.cost * 0.7) // 70% refund
    ...
    private handleEnemySpawn(request: any): void {
        const paths = this.state.paths && this.state.paths.length > 0 ? this.state.paths : [this.state.path]
        const route = request.routeIndex !== undefined && paths[request.routeIndex] ? paths[request.routeIndex] : this.state.path
    ...
    selectTowerAtScreen(screenX: number, screenY: number): { hitTower: boolean; towerId?: string; message: string } {
        const world = this.inputManager.screenToWorld(screenX, screenY, this.state.map)
        if (!world) {
            this.clearSelection()
        ...
        this.renderManager.setHoverState(
            tile ? { tile, position: tile.center, previewRange: tower.range, valid: true } : null
        )
        this.render()
        this.updateStore()
    ...
    placeTowerFromScreen(screenX: number, screenY: number, towerType: TowerType | null): PlacementResult {
        if (!towerType) {
    ...
    quickSetWave(waveIndex: number): void {
        this.state.currentWaveIndex = Math.max(0, Math.min(waveIndex, this.state.waves.length - 1))
        this.updateStore()
    }
    ...
    private updateStore(): void {
        ...
        useGameStore.getState().updateSnapshot({
            ...
            wave: {
                ...
                queued: waveStatus ? waveStatus.spawnQueue.length - waveStatus.nextIndex : 0,
            },
            wavePhase: this.state.wavePhase,
            nextWaveAvailable: this.state.wavePhase === 'idle' || this.state.wavePhase === 'completed',
