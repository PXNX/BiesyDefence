...
export class InputManager {
     ...
     */
    screenToWorld(screenX: number, screenY: number, map: MapData): Vector2 | null {
    ...
    private handleWheel(event: WheelEvent): void {
        ...
        const scale = this.viewportTransform?.scale ?? 1
        const worldBeforeX = (canvasX - (this.viewportTransform?.offsetX ?? 0)) / scale
        const worldBeforeY = (canvasY - (this.viewportTransform?.offsetY ?? 0)) / scale
