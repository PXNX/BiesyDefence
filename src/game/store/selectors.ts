...
export const selectIsRunning = () =>
  useGameStore(state => state.status === 'running');
export const selectIsPaused = () =>
  useGameStore(state => state.status === 'paused');
export const selectIsGameOver = () =>
  useGameStore(state => state.status === 'won' || state.status === 'lost');

// Wave info
export const selectWave = () => useGameStore(state => state.wave);
export const selectWavePhase = () => useGameStore(state => state.wavePhase);
export const selectNextWaveAvailable = () =>
  useGameStore(state => state.nextWaveAvailable);
export const selectAutoWaveEnabled = () =>
  useGameStore(state => state.autoWaveEnabled);
...
export const selectProjectileCount = () =>
  useGameStore(state => state.projectileCount);
...
export const selectShowHitboxes = () =>
  useGameStore(state => state.showHitboxes);
export const selectShowDamageNumbers = () =>
  useGameStore(state => state.showDamageNumbers);
export const selectSelectedTowerId = () =>
  useGameStore(state => state.selectedTowerId);
export const selectPreviewTowerType = () =>
  useGameStore(state => state.previewTowerType);
...
export const selectAchievements = () =>
  useGameStore(state => state.achievements);
export const selectAchievementNotifications = () =>
  useGameStore(state => state.achievementNotifications);
...
// Balance warnings
export const selectBalanceWarnings = () =>
  useGameStore(state => state.balanceWarnings);

// Active modifiers
export const selectActiveModifiers = () =>
  useGameStore(state => state.activeModifiers);

// Composite selectors
export const selectCanAffordTower = (cost: number) =>
  useGameStore(state => state.money >= cost);

export const selectWaveProgress = () =>
  useGameStore(state => ({
...
export const selectNextSpawnCountdown = () =>
  useGameStore(state => state.nextSpawnCountdown);
export const selectNextSpawnDelay = () =>
  useGameStore(state => state.nextSpawnDelay);
export const selectWavePreview = () => useGameStore(state => state.wavePreview);
export const selectLastWaveSummary = () =>
  useGameStore(state => state.lastWaveSummary);
