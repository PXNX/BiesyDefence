...
export const updateEnemies = (
  ...
  for (const event of modifierEvents) {
    ...
    if (enemy && !enemy.isDead) {
      if (event.type === 'dot' || event.type === 'burn') {
        ...
        const dealt = applyDamageToEnemy(enemy, damage, event.type === 'burn' ? 'burn' : 'dot')
  ...
  state.enemies.forEach((enemy) => {
    ...
    const slowAmount = modifierManager.calculateEffectiveValue(enemy.id, 'slow', 0)
    ...
    const speedMult = modifierManager.calculateEffectiveValue(enemy.id, 'speed_mult', 1)

    enemy.speedMultiplier = Math.max(0.1, (1 - slowAmount) * speedMult)

    // Calculate vulnerability
    enemy.vulnerability = modifierManager.calculateEffectiveValue(enemy.id, 'vulnerability', 0)
    enemy.armorReduction = modifierManager.calculateEffectiveValue(enemy.id, 'armor_reduction', 0)
    enemy.damageTakenMult = modifierManager.calculateEffectiveValue(enemy.id, 'damage_mult', 1)
