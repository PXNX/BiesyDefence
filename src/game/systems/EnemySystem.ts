...
/**
 * Enhanced enemy movement function that can work with EnemyEntity instances
 * This provides better integration with the new EnemyEntity class
 * @param state - Current game state
 * @param deltaSeconds - Time elapsed since last update in seconds
 */
export const updateEnemiesEnhanced = (state: GameState, deltaSeconds: number): void => {
  const path = state.path
  ...
  state.enemies.forEach((enemy) => {
    ...
    if (enemy.pathIndex >= path.length - 1) {
      if (!enemy.reachedGoal) {
        ...
        // Note: Life loss is now handled in GameController.cleanupEntities()
        // This prevents duplicate life loss and ensures proper state management
        console.log(`Enemy ${enemy.id} reached goal`)
      }
      return
    }
