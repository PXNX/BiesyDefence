  ...
  state.projectiles.forEach(projectile => {
    ...
    if (distToTarget <= travelDistance) {
      ...
      const hasPerk = (prefix: string) =>
        Boolean(
          sourceTower?.upgradeState?.perks?.some(id => id.includes(prefix))
        );
      ...
      state.particles.push(
        createHitMarker(target.position, projectile.color, Math.round(dealt))
      );
      ...
      if (sourceTower?.type === 'sativa' && hasPerk('shrapnel')) {
        ...
        state.particles.push(
          ...createSparkBurst(target.position, 'rgba(255, 234, 150, 0.9)', 6)
        );
        state.particles.push(
          ...createShrapnelImpact(
            target.position,
            projectile.splashRadius ?? 42
          )
        );
      }
      if (
        sourceTower?.type === 'sniper' &&
        (hasPerk('weak') || hasPerk('execution'))
      ) {
      ...
      if (sourceTower?.type === 'sativa' && hasPerk('shrapnel')) {
        ...
        const shrapnelTargets = findSplashTargets(
          target.position,
          state.enemies,
          shrapnelRadius,
          target.id
      ...
      if (target.health === 0) {
        target.isDead = true;
        state.particles.push(
          ...createImpactParticles(target.position, projectile.color)
        );
      } else {
        state.particles.push(
          ...createImpactParticles(target.position, '#fdf1a2')
        );
      }
