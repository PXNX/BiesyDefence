...
export class TelemetryCollector {
  ...
  public recordEnemySpawn(
    enemy: Enemy,
    waveIndex: number,
    elapsedWaveTime: number
  ): void {
    if (!this.enabled) {
      return;
    }
  ...
  public buildSnapshot(): TelemetrySnapshot {
    ...
    const towerSummaries: TelemetryTowerStats[] = Array.from(
      this.towerStats.values()
    ).map(stats => {
      ...
      return {
        ...
        overkillPercent:
          stats.totalDamage > 0
            ? (stats.totalOverkill / stats.totalDamage) * 100
            : 0,
      };
    });
  ...
  public getBalanceWarnings(): string[] {
    ...
    const hpRewardRatio =
      this.spawnStats.totalHp > 0
        ? this.spawnStats.totalReward / this.spawnStats.totalHp
        : null;
    if (
      hpRewardRatio !== null &&
