...
const selectTarget = (tower: Tower, enemies: Enemy[]): Enemy | null => {
    ...
    const inRange = enemies
      .filter(
        enemy =>
          !enemy.isDead &&
          !enemy.reachedGoal &&
          distanceBetween(tower.position, enemy.position) <= tower.range
      )
      .sort((a, b) => {
  ...
  if (targetingTime > 0.5) {
    // Log slow targeting operations
    logger.warn(
      'Slow targeting detected',
      {
...
const getProjectileSprite = (
  towerType: Tower['type'],
  damageType: DamageType,
  hasPerk: (s: string) => boolean
) => {
  if (towerType === 'support') {
    ...
    if (hasPerk('toxin'))
      return {
        key: 'projectile-toxin',
        size: 30,
        trailColor: '#9bffb0',
        trailWidth: 6,
      };
    return {
      key: 'projectile-support',
      size: 30,
      trailColor: '#6be8ff',
      trailWidth: 6,
    };
  ...
  if (towerType === 'sativa' || damageType === 'volley') {
    if (hasPerk('shrapnel'))
      return {
        key: 'projectile-shrapnel',
        size: 30,
        trailColor: '#fff7b2',
        trailWidth: 5,
      };
    return {
      key: 'projectile-volley',
      size: 26,
      trailColor: '#fdf1a2',
      trailWidth: 4,
    };
  ...
  if (towerType === 'chain') {
    return hasPerk('arc')
      ? {
          key: 'projectile-chain-arc',
          size: 34,
          trailColor: '#bcdcff',
          trailWidth: 6,
        }
      : {
          key: 'projectile-chain',
          size: 32,
          trailColor: '#bcdcff',
          trailWidth: 6,
        };
  ...
  if (towerType === 'sniper' || damageType === 'pierce') {
    return hasPerk('pierce') || hasPerk('weak')
      ? {
          key: 'projectile-pierce',
          size: 30,
          trailColor: '#ffdbb0',
          trailWidth: 4,
        }
      : {
          key: 'projectile-heavy',
          size: 30,
          trailColor: '#e0e0e0',
          trailWidth: 4,
        };
...
export const updateTowers = (
  ...
  state.towers.forEach(tower => {
    const hasPerk = (needle: string) =>
      Boolean(tower.upgradeState?.perks?.some(id => id.includes(needle)));
          ...
          candidates = state.enemies
            .filter(
              e =>
                !e.isDead &&
                !e.reachedGoal &&
                distanceBetween(tower.position, e.position) <= tower.range
            )
            .sort((a, b) => b.pathIndex - a.pathIndex);
        }
        if (candidates.length > 0) {
    ...
    if (tower.type === 'flamethrower') {
      if (tower.cooldown <= 0) {
        ...
        targets.forEach(enemy => {
          ...
          state.particles.push(
            ...createImpactParticles(enemy.position, '#fb923c')
          );
          if (tower.dot) {
            // Apply Burn Modifier
            modifierManager.addModifier(enemy.id, tower.id, {
    ...
    const baseDamage =
      tower.type === 'sativa' ? Math.floor(tower.damage * 0.6) : tower.damage;
    telemetry?.recordShot(tower);
    const spriteMeta = getProjectileSprite(
      tower.type,
      towerDamageType,
      hasPerk
    );
    for (let i = 0; i < projectileCount; i++) {
      const isCrit = tower.critChance
        ? Math.random() < tower.critChance
        : false;
      const critMult = isCrit ? (tower.critMultiplier ?? 1.5) : 1;
      const damagePerProjectile = Math.max(
        1,
        Math.floor(baseDamage * critMult)
      );
    ...
    state.particles.push(
      ...createMuzzleParticles(tower.position, target.position, tower.color)
    );
