...
export const queueEconomyEvent = (state: GameState, event: EconomyEvent): void => {
  ensureQueue(state).push(event)
}

const clampMoney = (value: number): number => Math.max(0, Math.min(999_999_999, value))
const clampScore = (value: number): number => Math.max(0, Math.min(999_999_999, value))
const clampLives = (value: number): number => Math.max(0, Math.min(999, value))
...
export const updateEconomy = (state: GameState, hooks?: EconomyHooks): EconomyDelta => {
  ...
  events.forEach((event) => {
    ...
    if (hooks?.onMoneyChange && delta.moneyDelta !== 0) {
      hooks.onMoneyChange(state.resources.money, delta.moneyDelta, event.reason)
    }
    if (hooks?.onScoreChange && delta.scoreDelta !== 0) {
      hooks.onScoreChange(state.resources.score, delta.scoreDelta, event.reason)
    }
    if (hooks?.onLivesChange && delta.livesDelta !== 0) {
      hooks.onLivesChange(state.resources.lives, delta.livesDelta, event.reason)
    }
  })
