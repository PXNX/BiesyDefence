...
const TOWER_TEXTURE_BY_TYPE: Record<
  TowerType,
  Record<1 | 2 | 3, TextureKey>
> = {
  ...
  flamethrower: {
    1: 'tower-flamethrower-l1',
    2: 'tower-flamethrower-l2',
    3: 'tower-flamethrower-l3',
  },
...
const TEXTURE_PATHS: Record<TextureKey, string> = {
  ...
  'tower-indica-l2-enhanced': assetPath(
    '/towers/tower_indica_level2_enhanced.png'
  ),
  ...
  'tower-sativa-l2-enhanced': assetPath(
    '/towers/tower_sativa_level2_enhanced.png'
  ),
  ...
  'tower-support-l2-enhanced': assetPath(
    '/towers/tower_support_level2_enhanced.png'
  ),
  ...
  'tower-flamethrower-l1': assetPath(
    '/towers/tower_flamethrower_build_level1.png'
  ),
  'tower-flamethrower-l2': assetPath(
    '/towers/tower_flamethrower_build_level2.png'
  ),
  'tower-flamethrower-l3': assetPath(
    '/towers/tower_flamethrower_build_level3.png'
  ),
  ...
  'tower-range-support': assetPath(
    '/effects/tower_range_indicator_support.png'
  ),
  'tower-placeholder':
  ...
  'effect-explosion-electric': assetPath(
    '/effects/effect_explosion_electric.png'
  ),
...
class TextureCache {
  ...
  private patternCache = new WeakMap<
    CanvasRenderingContext2D,
    Map<TextureKey, CanvasPattern>
  >();
...
export class CanvasRenderer {
  ...
  private drawTowerSprite(
    ...
    tower: {
      type: TowerType;
      level?: number;
      upgradeState?: { branch?: 'A' | 'B' };
    },
    tileSize: number
  ): void {
    const lvl = (tower.level ?? 1) as 1 | 2 | 3;
    const textureKey =
      TOWER_TEXTURE_BY_TYPE[tower.type]?.[lvl] ??
      TOWER_TEXTURE_BY_TYPE[tower.type]?.[1] ??
      'tower-placeholder';
  ...
  private drawEnemySprite(
    ...
    if (hasDot) {
      const hasBurn = enemy.effects.dot.some(
        effect => effect.damageType === 'burn'
      );
  ...
  private drawEnhancedEnemy(
    ...
    const baseRadius = Math.max(
      6,
      enemy.stats.radius * syncedScale * ENEMY_SCALE
    );
  ...
  private drawRangeVisualization(
    ...
    const ringImage = textureKey
      ? this.textureCache.getImage(textureKey)
      : null;
    if (ringImage && ringImage.complete && ringImage.naturalWidth > 0) {
      ...
      if (branch) {
        ...
        ctx.filter =
          branch === 'A' ? 'hue-rotate(180deg)' : 'hue-rotate(-20deg)';
      }
      const size = range * 2;
  ...
  render(
    ...
    this.drawParticles(
      ...
      particles,
      worldToScreen,
      debugSettings?.showDamageNumbers !== false,
      scale
    );
  ...
  private drawMap(
    ...
    ctx.save();
    const grassTextureKey =
      (map.theme?.grassTextureKey as TextureKey) ?? 'swampTerrain';
  ...
  private drawPathTiles(
    ...
    const pathTextureKey =
      (map.theme?.pathTextureKey as TextureKey) ?? 'pathStraight';
    ...
    map.tiles.forEach(tile => {
      ...
      const origin = worldToScreen(
        tile.grid.x * map.cellSize,
        tile.grid.y * map.cellSize
      );
  ...
  private drawSpecialTiles(
    ...
    map.specialTiles.forEach(special => {
      ...
      const textureKey = (
        special.type === 'gold_well' ? 'gold-well-tile' : 'rune-tile'
      ) as TextureKey;
  ...
  private drawParticles(
    ...
    particles.forEach(particle => {
      ...
      if (hasSprite && image) {
        ...
        const fps =
          particle.fps ??
          frameCount / Math.max(particle.maxLife || 0.001, 0.001);
        ...
        const sizePx =
          particle.size ??
          (particle.sizeWorld
            ? particle.sizeWorld * scale
            : Math.max(16, particle.radius * 2 || 32));
        const alpha = (particle.baseAlpha ?? 1) * lifeRatio;
        const rotation =
          particle.rotateToVelocity &&
            (particle.velocity.x !== 0 || particle.velocity.y !== 0)
