...
export class SpatialGrid {
  ...
  addEntity(entity: any): void {
    if (!entity.id || !entity.position) {
      logger.warn(
        'Invalid entity added to spatial grid',
        { entity },
        'performance'
      );
      return;
  ...
  findTargetsInRange(
    ...
    const validTargets = nearbyEnemies.filter(enemy => {
      ...
      if (
        includeFilters?.minPathIndex &&
        enemy.pathIndex < includeFilters.minPathIndex
      ) {
        return false;
      }
...
export function updateEnemySpatialGrid(enemies: Enemy[]) {
  ...
  if (updateTime > 1) {
    // Log if update takes more than 1ms
    logger.warn(
      'Slow spatial grid update',
      {
