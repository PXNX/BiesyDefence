...
export class InputValidator {
  ...
  validateNumber(value: any, rules?: Partial<ValidationRules>): ValidationResult {
  ...
  validateCoordinates(x: any, y: any, bounds?: { width: number; height: number }): ValidationResult {
  ...
  validateGameStateTransition(fromState: string, toState: string): ValidationResult {
    ...
    if (!allowed.includes(toState)) {
      logger.warn('Invalid game state transition', {
        from: fromState,
        to: toState,
        allowed
      }, 'input')
  ...
  validateTowerPlacement(
    ...
    if (x < margin || x > mapBounds.width - margin || 
        y < margin || y > mapBounds.height - margin) {
      return {
        isValid: false,
  ...
  private validateString(value: any, rules: ValidationRules, fieldName: string): ValidationResult {
...
export class InteractionDebouncer {
  ...
  isRapidInteraction(type: string): boolean {
    ...
    const threshold = this.thresholds[type as keyof typeof this.thresholds] || 20
    const isRapid = newCount > threshold

    if (isRapid) {
      logger.warn('Rapid interaction detected', {
        type,
        count: newCount,
        threshold,
...
export function sanitizeCanvasCoordinates(event: MouseEvent | TouchEvent): { x: number; y: number } | null {
