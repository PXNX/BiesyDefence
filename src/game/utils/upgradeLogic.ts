...
const applyPerk = (agg: AggregatedEffects, perk: UpgradePerk) => {
  ...
  agg.critMultiplier = Math.max(agg.critMultiplier, eff.critMultiplier ?? agg.critMultiplier)
...
export const recomputeTowerStats = (tower: Tower): Tower => {
  ...
  tower.chainFalloff = Math.max(0, (baseProfile.chainFalloff ?? 1) * agg.chainFalloffMult)
