...
import { GRID_WIDTH, GRID_HEIGHT, INITIAL_MONEY, TOWER_PROFILES } from '@/game/config/constants'
...
export class Chapter6IntegrationDemo {
  ...
  public static backwardCompatibilityTest(): void {
    ...
    try {
      console.log(`ℹ️ Old constants: ${GRID_WIDTH}x${GRID_HEIGHT}, $${INITIAL_MONEY}`)
      const legacyState = createInitialState()
      console.log(`ℹ️ Legacy state: ${legacyState.map.width}x${legacyState.map.height}`)
      const indicaTower = TOWER_PROFILES.indica
      console.log(`ℹ️ Tower profile: ${indicaTower.name} - ${indicaTower.description}`)
...
if (typeof window !== 'undefined') {
  ...
  console.log('ℹ️ Chapter 6 Demo available: run Chapter6IntegrationDemo.startProgressiveGame()')
}
